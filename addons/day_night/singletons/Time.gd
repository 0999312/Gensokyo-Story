...
const IN_GAME_SECONDS_PER_REAL_TIME_SECONDS: int = 720
...
func _ready():
	...
	_set_seconds_elapsed(get_start_time())
...
func _physics_process(delta):
	...
	if seconds_to_add >= 1:
		...
		_set_seconds_elapsed(get_start_time() + seconds_to_add)
...
func current_date_string():
	return str("%02d" % get_current_year()) + "/" + \
